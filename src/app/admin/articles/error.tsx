...
export default function AdminArticlesError({
  ...
  useEffect(() => {
    const msg = error?.message ? `Failed to load admin articles: ${error.message}` : "Failed to load admin articles";
    show(msg, { type: "error" });
  }, [error, show]);
  ...
  return (
    <section className="p-4 space-y-4" role="alert" aria-live="assertive">
      ...
      <p className="text-sm text-gray-700">
        The admin articles page encountered an error. You can try again or go back to the admin dashboard.
      </p>
      <div className="flex items-center gap-2">
        <button
          ...
          aria-label="Retry loading admin articles"
        >
          Retry
        </button>
        <button
